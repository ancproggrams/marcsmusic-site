...
const SOURCE_RECORD_URL_FIELDS = Object.freeze([
  "website",
  "submissionUrl",
  "instagramUrl",
  "spotifyUrl",
...
/**
 * Canonicalize an externally supplied source URL without guessing which
 * non-tracking query parameters are disposable. The returned value is always
 * an absolute HTTPS URL and is safe to use in semantic digests and CRM fields.
 */
...
export function canonicalizeSourceRecordUrls(record) {
  if (!record || typeof record !== "object" || Array.isArray(record)) return record;
  const canonical = { ...record };
  for (const field of SOURCE_RECORD_URL_FIELDS) {
    if (canonical[field] !== undefined) canonical[field] = canonicalizeSourceHttpsUrl(canonical[field]);
  }
  if (canonical.evidence && typeof canonical.evidence === "object" && !Array.isArray(canonical.evidence)) {
    canonical.evidence = {
      ...canonical.evidence,
      ...(canonical.evidence.url !== undefined
        ? { url: canonicalizeSourceHttpsUrl(canonical.evidence.url) }
        : {})
    };
  }
  return canonical;
...
export function canonicalizeSourceRecords(records) {
  if (!Array.isArray(records)) return records;
  return records.map(canonicalizeSourceRecordUrls);
}
