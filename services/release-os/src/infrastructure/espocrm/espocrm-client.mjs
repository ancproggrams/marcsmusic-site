export class EspoCrmClient {
  constructor(options = {}) {
    this.baseUrl = stripTrailingSlash(options.baseUrl ?? process.env.ESPOCRM_BASE_URL);
    this.apiKey = options.apiKey ?? process.env.ESPOCRM_API_KEY;
    this.fetch = options.fetch ?? globalThis.fetch;
    this.timeoutMs = options.timeoutMs ?? parsePositiveInteger(process.env.ESPOCRM_TIMEOUT_MS, 10_000);
  ...
  async listContacts() {
    ...
    try {
      ...
      if (!response.ok) {
        throw Object.assign(new Error(`EspoCRM returned HTTP ${response.status}`), {
          statusCode: 502,
          code: "ESPOCRM_REQUEST_FAILED"
        });
      }
