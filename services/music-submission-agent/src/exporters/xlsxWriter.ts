...
function worksheetXml(rows: Record<string, unknown>[]): string {
  ...
  const tableRows = headers.length > 0 ? [Object.fromEntries(headers.map((header) => [header, header])), ...rows] : [];
  const sheetData = tableRows
    .map((row, rowIndex) => {
      const cells = headers
        .map((header, columnIndex) => {
          const cellRef = `${columnName(columnIndex)}${rowIndex + 1}`;
          return `<c r="${cellRef}" t="inlineStr"><is><t>${escapeXml(row[header])}</t></is></c>`;
        })
        .join('');
...
export async function writeMinimalXlsx(filePath: string, rows: Record<string, unknown>[]): Promise<void> {
  const files: ZipFile[] = [
    {
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets><sheet name="verified_platforms" sheetId="1" r:id="rId1"/></sheets>
</workbook>`)
    },
    {
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/>
    ...
    },
    {
      ...
      data: Buffer.from(worksheetXml(rows))
    }
  ];
