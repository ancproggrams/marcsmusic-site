...
test("cancelled bookings cannot acquire a calendar fulfillment claim", () => {
  const decision = claimCalendarFulfillment(
    { id: BOOKING_ID, status: "cancelled" },
    { uid: `marcsmusic-${BOOKING_ID}@marcsmusic.nl`, url: `https://calendar.example/${BOOKING_ID}.ics` },
    { token: "claim-token", now: new Date("2030-01-01T00:00:00.000Z"), leaseMs: 30_000 }
  );
  assert.equal(decision.kind, "terminal");
});

test("a paid webhook after cancellation requires refund review and never recreates the event", async (t) => {
  const fixture = await integrationFixture(t, { bookingStatus: "cancelled" });
  fixture.state.payment = validMolliePayment({ status: "paid" });
  const response = await postWebhook(fixture.baseUrl, PAYMENT_ID);
  assert.equal(response.status, 200);
  assert.equal((await fixture.readDb()).bookings[0].status, "refund_review");
...
async function integrationFixture(t, options = {}) {
  ...
  const seeded = bookingDatabase(upstream.baseUrl, options.crashedPut, options.bookingStatus);
...
async function startUpstream(options) {
  const counts = { mollieGet: 0, molliePost: 0, crm: 0, calendarReport: 0, calendarPut: 0, calendarGet: 0 };
  const state = {
    counts,
    ...
    try {
      if (request.method === "POST" && url.pathname === "/v2/payments") {
        ...
        state.payment = {
          ...
          _links: { checkout: { href: "https://checkout.example.test/support" } }
        };
        return sendJson(response, 201, state.payment);
      }
...
function bookingDatabase(upstreamBaseUrl, crashedPut = false, bookingStatus = null) {
  const booking = {
    id: BOOKING_ID,
    status: bookingStatus || (crashedPut ? "paid_calendar_pending" : "pending_payment"),
