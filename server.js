...
const bookingSqlitePath = process.env.BOOKING_SQLITE_PATH || (process.env.RAILWAY_ENVIRONMENT ? join(dirname(bookingDbPath), "bookings.sqlite") : "");
const sqlStorageEnabled = Boolean(process.env.DATABASE_URL || bookingSqlitePath);
...
if (!Number.isSafeInteger(slotStepMinutes) || slotStepMinutes <= 0) {
  throw new Error("BOOKING_SLOT_STEP_MINUTES must be a positive integer.");
}
...
const transparanteBrokerSyncEnabled = process.env.TRANSPARANTE_BROKER_SYNC_ENABLED === "true";
const transparanteBrokerSyncIntervalMinutes = Math.max(15, numberFromEnv("TRANSPARANTE_BROKER_SYNC_INTERVAL_MINUTES", 60));
...
if (!Number.isSafeInteger(pendingHoldMinutes) || pendingHoldMinutes <= 0) throw new Error("BOOKING_PENDING_HOLD_MINUTES must be positive.");
if (!Number.isSafeInteger(bookingBufferMinutes) || bookingBufferMinutes < 0) throw new Error("BOOKING_BUFFER_MINUTES cannot be negative.");
if (!Number.isSafeInteger(minLeadHours) || minLeadHours < 0) throw new Error("BOOKING_MIN_LEAD_HOURS cannot be negative.");
for (const type of bookingTypes) {
  if (!Number.isSafeInteger(type.durationMinutes) || type.durationMinutes <= 0) throw new Error(`Booking duration for ${type.id} must be positive.`);
  if (!Number.isSafeInteger(type.priceCents) || type.priceCents < 0) throw new Error(`Booking price for ${type.id} cannot be negative.`);
}
try {
  new Intl.DateTimeFormat("en", { timeZone: bookingTimeZone }).format();
} catch {
  throw new Error("BOOKING_TIMEZONE must be a valid IANA time zone.");
}
const validationDay = "2030-01-15";
...
async function ensureDbFile() {
  ...
  await writeFile(
    bookingDbPath,
    JSON.stringify({ bookings: [], supports: [], payments: [], newsletterSubscriptions: [], assignments: [], syncState: {}, trackPlayCounts: defaultTrackPlayCounts(), audit: [] }, null, 2),
    "utf8"
  );
}

async function readDb() {
  if (bookingStore) return readSqlSnapshot(bookingStore);
  await ensureDbFile();
...
async function writeDb(db) {
  if (bookingStore) {
    await bookingStore.transaction((tx) => writeSqlSnapshot(tx, db));
    return;
  }
  await mkdir(dirname(bookingDbPath), { recursive: true });
...
function queueDbWork(work, { expire = true } = {}) {
  const next = dbQueue.then(async () => {
    if (bookingStore) {
      return bookingStore.transaction(async (tx) => {
        const db = await readSqlSnapshot(tx);
        if (expire) expireOldPendingBookings(db);
        const result = await work(db);
        await writeSqlSnapshot(tx, db);
        return result;
      });
    }
    const release = await acquireDbFileLock();
    try {
...
async function listAllRecords(api, table) {
  const records = [];
  for (let offset = 0; ; offset += 500) {
    const page = await api.list(table, { limit: 500, offset });
    records.push(...page);
    if (page.length < 500) return records;
  }
}

async function readSqlSnapshot(api) {
  const [bookings, supports, payments, newsletterSubscriptions, assignments, audit, syncRecords, trackCounts] = await Promise.all([
    listAllRecords(api, "bookings"),
    listAllRecords(api, "supports"),
    listAllRecords(api, "payments"),
    listAllRecords(api, "subscriptions"),
    listAllRecords(api, "assignments"),
    api.list("audit_events", { limit: 1000 }),
    listAllRecords(api, "sync_state"),
    api.get("metadata", "track-play-counts")
  ]);
  return {
    bookings,
    supports,
    ...
    assignments,
    syncState: Object.fromEntries(syncRecords.map((entry) => [entry.id, entry])),
    trackPlayCounts: normalizeTrackPlayCounts(trackCounts?.counts),
    audit
  };
}

async function syncCollection(api, table, records, idFor) {
  const normalized = records.map((record) => ({ ...record, id: idFor(record) }));
  const keep = new Set(normalized.map((record) => record.id));
  for (const existing of await listAllRecords(api, table)) {
    if (!keep.has(existing.id)) await api.remove(table, existing.id);
  }
  for (const record of normalized) await api.put(table, record);
}

async function writeSqlSnapshot(api, db) {
  await syncCollection(api, "bookings", db.bookings || [], (record) => record.id);
  await syncCollection(api, "supports", db.supports || [], (record) => record.id);
  await syncCollection(api, "payments", db.payments || [], (record) => record.id);
  await syncCollection(api, "subscriptions", db.newsletterSubscriptions || [], (record) => record.id || record.email);
  await syncCollection(api, "assignments", db.assignments || [], (record) => record.id);
  await syncCollection(api, "audit_events", db.audit || [], (record) => record.id);
  await syncCollection(api, "sync_state", Object.entries(db.syncState || {}).map(([id, record]) => ({ ...record, id })), (record) => record.id);
  await api.put("metadata", { id: "track-play-counts", counts: normalizeTrackPlayCounts(db.trackPlayCounts) });
}

async function acquireDbFileLock() {
  const lockPath = `${bookingDbPath}.lock`;
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw Object.assign(new Error("Booking storage is busy."), { statusCode: 503 });
      }
      await new Promise((resolveWait) => setTimeout(resolveWait, 25 + Math.floor(Math.random() * 25)));
    }
  }
}
...
async function handleApi(request, response, url) {
  ...
  if (request.method === "GET" && url.pathname === "/api/health/ready") {
    ...
    let recoveryBacklog = 0;
    let assignmentSyncFresh = !transparanteBrokerSyncEnabled;
    try {
      const db = await readDb();
      recoveryBacklog = db.bookings.filter((booking) => ["paid_calendar_pending", "cancel_requested"].includes(booking.status)).length
        + db.newsletterSubscriptions.filter((subscription) => subscription.crmStatus === "pending_retry").length;
      const sync = db.syncState["de-transparante-broker"];
      assignmentSyncFresh ||= sync?.status === "ok"
        && Date.parse(sync.completedAt) > Date.now() - 2 * transparanteBrokerSyncIntervalMinutes * 60_000;
    } catch {
      storageReady = false;
    }
    const integrations = {
    ...
    const ready = storageReady && Object.values(integrations).every(Boolean) && recoveryBacklog === 0 && assignmentSyncFresh;
  ...
  if (request.method === "GET" && url.pathname === "/api/health") {
    ...
    sendJson(response, storageReady ? 200 : 503, {
  ...
  if (request.method === "POST" && url.pathname === "/api/booking/create") {
    const booking = await createBooking(await readJsonBody(request), request.headers["idempotency-key"]);
  ...
  if (request.method === "GET" && url.pathname === "/api/admin/assignments") {
    ...
    const assignments = db.assignments.filter((entry) => !source || entry.source === source).slice(offset, offset + 100);
    sendJson(response, 200, { assignments, syncState: db.syncState["de-transparante-broker"] || null });
    return;
  }

  if (request.method === "POST" && url.pathname === "/api/admin/assignments/sync") {
    if (!requireAdmin(request, response)) return;
    sendJson(response, 200, await syncAssignments());
    return;
  }
...
async function subscribeNewsletter(input, request) {
  ...
  await withDb(async (db) => {
    ...
    const record = {
      ...
      crmStatus,
      crmContactId,
      crmAttempts: crmStatus === "pending_retry" ? (existing?.crmAttempts || 0) : 0,
      crmNextRetryAt: crmStatus === "pending_retry" ? now : null,
      updatedAt: now
    };
...
async function createBooking(input, rawRequestKey) {
  const requestKey = rawRequestKey ? String(rawRequestKey) : randomUUID();
  if (!/^[A-Za-z0-9._:-]{16,128}$/.test(requestKey)) {
    throw Object.assign(new Error("Een geldige idempotency-key is verplicht."), { statusCode: 400 });
  }
  const prior = (await readDb()).bookings.find((entry) => entry.requestKey === requestKey);
  if (prior?.checkoutUrl) {
    return { bookingId: prior.id, status: prior.status, checkoutUrl: prior.checkoutUrl, expiresAt: prior.expiresAt };
  }
  ...
  const createdResult = await withDb(async (db) => {
    const existingRequest = db.bookings.find((entry) => entry.requestKey === requestKey);
    if (existingRequest) return { booking: { ...existingRequest }, existing: true };
    const localBusy = getReservedIntervals(db);
  ...
  const created = createdResult.booking;
  if (createdResult.existing) {
    if (created.checkoutUrl) {
      return { bookingId: created.id, status: created.status, checkoutUrl: created.checkoutUrl, expiresAt: created.expiresAt };
    }
    throw Object.assign(new Error("Deze boekingsaanvraag wordt al verwerkt. Probeer het zo opnieuw."), { statusCode: 409 });
  }

  try {
    const crmContact = await createOrUpdateCrmContact(created.customer);
    ...
    await updateCrmBookingRecord({ ...created, crmBookingId: crmBooking.id }, { molliePaymentId: payment.id, molliePaymentStatus: payment.status || "open" })
      .catch((error) => console.error(`CRM payment sync failed: ${publicErrorMessage(error)}`));
    ...
    const recovery = await withDb(async (db) => {
      const booking = db.bookings.find((entry) => entry.id === bookingId);
      if (!booking) {
        return null;
      }
      if (booking.checkoutUrl && booking.molliePaymentId) {
        audit(db, "booking.setup_degraded", { bookingId, error: publicErrorMessage(error) });
        return { bookingId, status: booking.status, checkoutUrl: booking.checkoutUrl, expiresAt: booking.expiresAt };
      }
...
async function handleMollieWebhook(payload) {
  ...
  try {
    committed = await withDb(async (db) => {
      ...
      if (effectiveStatus === "paid" && ["cancelled", "cancel_requested"].includes(current.booking.status)) {
        current.booking.status = "refund_review";
        current.booking.updatedAt = now;
        audit(db, "booking.paid_after_cancellation", { bookingId: current.booking.id, reasonCode: "REFUND_REVIEW_REQUIRED" });
      }

      const protectedStatus = new Set(["confirmed", "paid_calendar_pending", "manual_review", "refund_review"]);
...
function getReservedIntervals(db, excludeBookingId = null) {
  ...
  return db.bookings
    ...
    .map((booking) => ({
      start: new Date(booking.startUtc),
      end: new Date(booking.endUtc)
    }));
}
...
function parseBusyIntervalsFromCalendarXml(xml, rangeStart, rangeEnd) {
  return parseCalendarResponse(xml, rangeStart, rangeEnd, bookingTimeZone)
...
async function createOrUpdateCrmContact(input) {
  ...
  if (existing?.id) {
    ...
    if (!input.phone) delete payload.phoneNumber;
    payload.description = [existing.description, payload.description].filter(Boolean).join("\n");
    if (input.newsletterOptIn === undefined) {
      delete payload.newsletterOptIn;
      delete payload.consentAt;
...
async function cancelBooking(bookingId) {
  ...
  try {
    if (snapshot.caldavEventUid) await deleteCalDavEvent(snapshot.caldavEventUid);
    await updateCrmBookingRecord(snapshot, { status: "cancelled" });
  } catch (error) {
    console.error(`Cancellation sync failed: ${publicErrorMessage(error)}`);
    await withDb(async (db) => {
      const booking = db.bookings.find((entry) => entry.id === bookingId);
      if (!booking || booking.status !== "cancel_requested") return;
      const attempts = (booking.cancellationRetry?.attempts || 0) + 1;
      booking.cancellationRetry = {
        attempts,
        lastError: publicErrorMessage(error).slice(0, 300),
        nextRetryAt: new Date(Date.now() + retryDelayMs(attempts)).toISOString()
      };
      audit(db, "booking.cancellation_retry_scheduled", { bookingId, attempts });
    });
    return { ok: false, pending: true, bookingId };
  }
  ...
  await withDb(async (db) => {
    ...
    booking.cancellationRetry = null;
    booking.updatedAt = new Date().toISOString();
    audit(db, "booking.cancellation_completed", { bookingId });
  });
...
async function runDurableRecovery() {
  if (recoveryRunning) return;
  recoveryRunning = true;
  try {
    const db = await readDb();
    const now = Date.now();
    for (const booking of db.bookings.filter((entry) => entry.status === "paid_calendar_pending" && calendarRetryDue(entry, now)).slice(0, 20)) {
      await confirmPaidBooking(booking.id).catch((error) => console.error(`Calendar recovery failed: ${publicErrorMessage(error)}`));
    }
    for (const booking of db.bookings.filter((entry) => entry.status === "cancel_requested" && Date.parse(entry.cancellationRetry?.nextRetryAt || 0) <= now).slice(0, 20)) {
      await cancelBooking(booking.id).catch((error) => console.error(`Cancellation recovery failed: ${publicErrorMessage(error)}`));
    }
    for (const subscription of db.newsletterSubscriptions.filter((entry) => entry.crmStatus === "pending_retry" && Date.parse(entry.crmNextRetryAt || 0) <= now).slice(0, 20)) {
      try {
        const contact = await createOrUpdateCrmContact(subscription);
        await addContactToNewsletterList(contact.id);
        await withDb(async (currentDb) => {
          const current = currentDb.newsletterSubscriptions.find((entry) => entry.email === subscription.email);
          if (!current || current.crmStatus !== "pending_retry") return;
          current.crmStatus = "synced";
          current.crmContactId = contact.id || null;
          current.crmAttempts = 0;
          current.crmNextRetryAt = null;
          current.updatedAt = new Date().toISOString();
          audit(currentDb, "newsletter.retry_succeeded", { email: current.email, crmContactId: current.crmContactId });
        });
      } catch (error) {
        console.error(`Newsletter recovery failed: ${publicErrorMessage(error)}`);
        await withDb(async (currentDb) => {
          const current = currentDb.newsletterSubscriptions.find((entry) => entry.email === subscription.email);
          if (!current || current.crmStatus !== "pending_retry") return;
          current.crmAttempts = (current.crmAttempts || 0) + 1;
          current.crmNextRetryAt = new Date(Date.now() + retryDelayMs(current.crmAttempts)).toISOString();
          current.lastCrmError = publicErrorMessage(error).slice(0, 300);
          audit(currentDb, "newsletter.retry_failed", { email: current.email, attempts: current.crmAttempts });
        });
      }
    }
  } finally {
...
function retryDelayMs(attempts) {
  return Math.min(60 * 60_000, 1_000 * 2 ** Math.min(12, Math.max(1, attempts))) + Math.floor(Math.random() * 1_000);
}

function calendarRetryDue(booking, now) {
  const attempt = Math.max(1, booking.calendarFulfillment?.attempt || 1);
  const updatedAt = Date.parse(booking.calendarFulfillment?.updatedAt || booking.updatedAt || 0);
  return !Number.isFinite(updatedAt) || updatedAt + retryDelayMs(attempt) <= now;
}

async function syncAssignments() {
  const startedAt = new Date().toISOString();
  const incoming = await fetchTransparanteBrokerAssignments({ baseUrl: transparanteBrokerBaseUrl });
  return withDb(async (db) => {
    db.assignments = mergeAssignments(db.assignments || [], incoming, startedAt);
    const state = {
      status: "ok",
      startedAt,
      completedAt: new Date().toISOString(),
      receivedCount: incoming.length,
      activeCount: db.assignments.filter((entry) => entry.source === "de-transparante-broker" && entry.active).length
    };
    db.syncState["de-transparante-broker"] = state;
    audit(db, "assignments.sync_completed", { receivedCount: state.receivedCount, activeCount: state.activeCount });
...
if (sqlStorageEnabled) {
  bookingStore = await openStore({
    url: process.env.DATABASE_URL,
    path: bookingSqlitePath || join(root, "data", "bookings.sqlite"),
    legacyPath: bookingDbPath
  });
}
...
server.listen(port, "0.0.0.0", () => {
  ...
  void runDurableRecovery();
});

const recoveryTimer = setInterval(() => void runDurableRecovery(), 30_000);
recoveryTimer.unref();
let assignmentSyncRunning = false;
const scheduleAssignmentSync = async () => {
  if (assignmentSyncRunning) return;
  assignmentSyncRunning = true;
  try {
    await syncAssignments();
  } catch (error) {
    console.error(`Assignment sync failed: ${publicErrorMessage(error)}`);
  } finally {
    assignmentSyncRunning = false;
  }
};
if (transparanteBrokerSyncEnabled) void scheduleAssignmentSync();
const assignmentSyncTimer = transparanteBrokerSyncEnabled
  ? setInterval(() => void scheduleAssignmentSync(), transparanteBrokerSyncIntervalMinutes * 60_000)
  : null;
assignmentSyncTimer?.unref();
...
async function shutdown(signal) {
  ...
  if (assignmentSyncTimer) clearInterval(assignmentSyncTimer);
  const forceTimer = setTimeout(() => process.exit(1), 10_000);
  forceTimer.unref();
  await new Promise((resolveClose) => server.close(resolveClose));
  await bookingStore?.close();
  console.log(`MarcsMusic site stopped after ${signal}`);
}
