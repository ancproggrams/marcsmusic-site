...
const tables = new Set(['bookings', 'payments', 'subscriptions', 'assignments', 'jobs', 'audit_events', 'sync_state', 'metadata']);
...
export async function openStore({ url, path, legacyPath } = {}) {
  ...
  if (legacyPath) {
    ...
    if (legacy) await store.transaction(async tx => {
      ...
      for (const [key, table] of [['bookings','bookings'], ['payments','payments'], ['newsletterSubscriptions','subscriptions'], ['assignments','assignments'], ['audit','audit_events']]) {
